...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (reviewsError) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to fetch reviews", details: reviewsError }),
        { status: 500, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    if (!reviews || reviews.length === 0) {
      return new Response(
        JSON.stringify({ message: "No reviews need sentiment analysis", processed: 0, failed: 0, errors: [] }),
...
Example response: {"sentiment": "positive", "score": 85, "emotions": ["😊", "👍"]}`
        ...
        const { error: insertError } = await supabaseClient
          ...
          .insert({
            ...
            emotions: analysisResult.emotions ? { emoticons: analysisResult.emotions } : null,
        ...
        result.processed++;
        console.log(`Processed review ${review.id}: ${analysisResult.sentiment} (${analysisResult.score})`);
    ...
    console.log(`Sentiment analysis complete: ${result.processed} processed, ${result.failed} failed`);
